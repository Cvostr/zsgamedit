...
void MainWin::addProjectToVector(ProjectConf conf){
    ...
    std::string project_label = conf.projLabel.toStdString();
    std::string project_root_path = conf.projectRootPath.toStdString();

    //Copy required data
    conf.editwin_proj.label = project_label;
    conf.editwin_proj.root_path = project_root_path;
...
void MainWin::saveProjectsConfiguration(){
    ...
    if (!conf_stream.is_open())
        throw std::runtime_error("Could not open projs.inf file stream");
...
void ProjectCtxMenu::runEngineClicked(ZSGAPI gapi){
    ...
    game_info.app_version = 0; //Setting base app version to 0
    game_info.game_perspective = conf->editwin_proj.perspective; //Setting perspective
    game_info.blob_root_path = conf->projectRootPath.toStdString() + "/.build/";
